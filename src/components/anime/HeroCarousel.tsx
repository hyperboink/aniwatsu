...
export default function HeroCarousel({ items }: { items: Anime[] }) {
  ...
  return (
    <div className="relative" aria-label="Featured anime carousel" role="region">
      <div className="relative h-[70vh] min-h-[480px] max-h-[720px] px-6 md:px-0 overflow-hidden">
        {/* Background */}
        <div
          ref={bgRef}
        ...
        {/* Content */}
        <div
          className={`relative h-full max-w-7xl mx-auto px-6 md:px-4 flex flex-col justify-center pb-16 transition-all duration-300 ${
        ...
        <button
          ...
          className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/40 hover:bg-black/70 flex items-center justify-center text-white border border-white/10"
        >
        ...
        <button
          ...
          className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/40 hover:bg-black/70 flex items-center justify-center text-white border border-white/10"
        >
          ...
          {items.map((_, i) => (
            ...
            >
              <span className={`block rounded-full transition-all duration-300 ${
                i === idx ? "w-5 h-1.5 bg-violet-400" : "w-1.5 h-1.5 bg-white/60 hover:bg-white/80"
              }`} />
            </button>
          ))}
        </div>
